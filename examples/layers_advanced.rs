...
fn main() {
    ...
    let destination = Ipv4Address::new(10, 20, 30, 40);
    let decision = table.select_route(destination).expect("route exists");

    println!("{destination} sale por {}", decision.interface);
    println!("prefijo ganador: {}", decision.matched_prefix);
}
